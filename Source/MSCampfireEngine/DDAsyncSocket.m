...
//  DDAsyncSocket.m
//  
//  Renamed from AsyncSocket.m because it was colliding with older AsyncSocket code included in Adium 1.4 source.
//
...
NSString *const DDAsyncSocketException = @"AsyncSocketException";
NSString *const DDAsyncSocketErrorDomain = @"AsyncSocketErrorDomain";
...
@interface DDAsyncSocket (Private)
 ...
 **/
@interface DDAsyncReadPacket : NSObject
{
@public
...
@implementation DDAsyncReadPacket
 ...
 **/
@interface DDAsyncWritePacket : NSObject
{
@public
	NSData *buffer;
	CFIndex bytesDone;
...
@implementation DDAsyncWritePacket
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(CFIndex *)done total:(CFIndex *)total
{
	...
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[DDAsyncReadPacket class]]) return NAN;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(CFIndex *)done total:(CFIndex *)total
{
	...
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[DDAsyncWritePacket class]]) return NAN;
...
- (BOOL)acceptOnInterface:(NSString *)interface port:(UInt16)port error:(NSError **)errPtr
{
	if (theDelegate == NULL)
  {
		[NSException raise:DDAsyncSocketException
...
- (void)doAcceptWithSocket:(CFSocketNativeHandle)newNative
{
	...
	DDAsyncSocket *newSocket = [[[[self class] alloc] initWithDelegate:theDelegate] autorelease];
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[DDAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:DDAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
}
...
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:DDAsyncSocketErrorDomain code:AsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getConnectTimeoutError
{
	...
	return [NSError errorWithDomain:DDAsyncSocketErrorDomain code:AsyncSocketConnectTimeoutError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:DDAsyncSocketErrorDomain code:AsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:DDAsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:info];
}
...
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:DDAsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:info];
}
...
- (void)readDataToLength:(CFIndex)length withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	DDAsyncReadPacket *packet = [[DDAsyncReadPacket alloc] initWithData:buffer
                                                          timeout:timeout
                                                              tag:tag
                                                 readAllAvailable:NO
                                                       terminator:nil
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	DDAsyncWritePacket *packet = [[DDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
                  ...
                  withData:(const void *)pData
{
	...
	switch (type)
	{
		...
		case kCFSocketAcceptCallBack:
			...
			break;
		default:
			NSLog (@"AsyncSocket %p received unexpected CFSocketCallBackType %llu.", self, (unsigned long long) type);
			break;
	}
}
...
- (void)doCFReadStreamCallback:(CFStreamEventType)type forStream:(CFReadStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"AsyncSocket %p received unexpected CFReadStream callback, CFStreamEventType %llu.", self, (unsigned long long) type);
	}
}
...
- (void)doCFWriteStreamCallback:(CFStreamEventType)type forStream:(CFWriteStreamRef)stream
{
	...
	switch (type)
	{
		...
		case kCFStreamEventEndEncountered:
			...
			break;
		default:
			NSLog (@"AsyncSocket %p received unexpected CFWriteStream callback, CFStreamEventType %llu.", self, (unsigned long long) type);
	}
}
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	...
	DDAsyncSocket *theSocket = [[(DDAsyncSocket *)pInfo retain] autorelease];
